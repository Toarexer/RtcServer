...
[UsedImplicitly]
public record AuthorizationRequest(string Username, string Password, string RemoteAddress);
...
[UsedImplicitly]
public record AppInfo(string Environment, string Version);
...
[UsedImplicitly]
public record RtcClientInfo(string? Alias, uint Channel, string? Remote);
...
public class RtcClientInfos<TRtcClient> : Dictionary<uint, RtcClientInfo> where TRtcClient : IRtcClient {
	public RtcClientInfos(IEnumerable<KeyValuePair<TRtcClient, uint>> items) {
		foreach (KeyValuePair<TRtcClient, uint> item in items)
			Add(item.Key.Id, new RtcClientInfo(item.Key.Alias, item.Value, item.Key.Remote));
	}
}
...
/// <param name="NextClientId">The ID to be assigned to the next client.</param>
/// <param name="UpTime">The time elapsed since the creation of the <see cref="RtcClientStore{T}"/>.</param>
[UsedImplicitly]
public record RtcClientStoreInfo(int ChannelCount, int ClientCount, uint NextClientId, TimeSpan UpTime);
...
/// <param name="Clients">The current <see cref="RtcClientInfos{T}"/>.</param>
[UsedImplicitly]
public record AllInfo<TRtcClient>(AppInfo App, Config Config, RtcClientStoreInfo Store, RtcClientInfos<TRtcClient> Clients) where TRtcClient : IRtcClient;
